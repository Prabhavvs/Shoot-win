...
class SweetShotGame {
    constructor(options = {}) {
        ...
        this.gameWidth = this.canvas.width || 390;
        this.gameHeight = this.canvas.height || 700;
    ...
    async loadImages() {
        const bowPromise = this.loadOneImage("assets/ui/bow.png", "high");
        const arrowPromise = this.loadOneImage("assets/ui/arrow.png", "high");

        const targetPromises = TARGET_IMAGE_PATHS.map((src) => this.loadOneImage(src));
        ...
        const [bowImage, arrowImage, loadedTargets, loadedIntruders] = await Promise.all([
            bowPromise,
            arrowPromise,
            Promise.all(targetPromises),
            Promise.all(intruderPromises)
        ]);

        this.bowImage = bowImage;
        this.arrowImage = arrowImage;
        this.images = loadedTargets.filter(Boolean);
        this.intruderImages = loadedIntruders.filter(Boolean);
    }

    loadOneImage(src, fetchPriority = "auto") {
    ...
    updateTargets(delta) {
        this.targets.forEach((target) => {
            ...
            if (target.x > this.gameWidth + target.size) {
    ...
    draw() {
        ...
        const bowX = this.gameWidth / 2;
        const bowY = this.getBowY();
        const bowScale = this.getBowScale();
        if (this.bowImage) {
        ...
        const stringLeftX = bowX - 126 * bowScale;
        const stringRightX = bowX + 118 * bowScale;
        ...
        const arrowWidth = 30 * bowScale;
        const arrowHeight = 140 * bowScale;
        if (this.arrowImage) {
        ...
        for (const arrow of this.arrows) {
            if (this.arrowImage) {
                ...
                this.ctx.drawImage(
                    ...
                    arrowCrop.sw,
                    arrowCrop.sh,
                    arrow.x - arrowWidth / 2,
                    arrow.y - arrowHeight * 0.56,
                    arrowWidth,
                    arrowHeight
                );
            } else {
    ...
    getDragPullLimit() {
        const compact = this.getCompactFactor();
        const touchPenalty = this.isTouchDevice() ? 8 : 0;
        const baseLimit = Math.max(90, 130 - compact * 30 - touchPenalty);
        const stage = document.getElementById("gameStage");
        if (!stage || !this.dragControlWrap) return baseLimit;

        // Keep drag translation fully inside stage so it never drops into hint strip.
        const stageRect = stage.getBoundingClientRect();
        const wrapRect = this.dragControlWrap.getBoundingClientRect();
        const bottomBuffer = 10;
        const availableDown = Math.max(0, stageRect.bottom - wrapRect.bottom - bottomBuffer);
        return Math.max(22, Math.min(baseLimit, availableDown));
    }

    getBowY() {
        const compact = this.getCompactFactor();
        const touchLift = this.isTouchDevice() ? 16 : 0;
        return this.gameHeight - (256 + compact * 42 + touchLift);
    }

    setViewportHeightVar() {
        const vh = window.visualViewport ? window.visualViewport.height : window.innerHeight;
        if (!vh) return;
        document.documentElement.style.setProperty("--viewport-height", `${Math.round(vh)}px`);
    }
    ...
    getCompactFactor() {
        const vh = this.getViewportHeight() || 820;
        return Math.max(0, Math.min(1, (820 - vh) / 240));
    }

    getBowScale() {
        const compact = this.getCompactFactor();
        const touchAdjustment = this.isTouchDevice() ? 0.04 : 0;
        return Math.max(0.82, 1 - compact * 0.14 - touchAdjustment);
    }

    setupHiDPICanvas() {
        const nextDpr = Math.max(1, window.devicePixelRatio || 1);
        this.devicePixelRatio = nextDpr;
        this.canvas.width = Math.round(this.gameWidth * nextDpr);
        this.canvas.height = Math.round(this.gameHeight * nextDpr);
    }
    ...
    updateDragAnchorPosition() {
        ...
        const anchorCanvasX = this.gameWidth / 2;
        ...
        const wrapHeight = this.dragControlWrap.offsetHeight || 70;
        const minTop = 8;
        const maxTop = Math.max(minTop, stageRect.height - wrapHeight - 8);
        const nextTop = anchorStageY - wrapHeight / 2;
        const clampedTop = Math.max(minTop, Math.min(maxTop, nextTop));

        this.dragControlWrap.style.left = `${anchorStageX}px`;
        this.dragControlWrap.style.top = `${clampedTop}px`;
